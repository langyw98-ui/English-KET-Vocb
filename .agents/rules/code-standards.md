...
1. 函数与模块粒度：函数保持单一职责，超过约 80 行或包含 3 个以上并列分支的函数必须按职责拆分。但拆分必须兼顾模块内聚性，禁止按“一函数一文件”机械地过度拆分。
...
6. 高内聚与防文件碎片化（Common Closure Principle, CCP）：
   - 共同封闭原则（CCP）：因相同原因修改、协同完成同一子任务的代码应当内聚在同一模块中。禁止将紧密耦合的内部处理步骤过度拆分为多个缺乏独立复用价值的微型文件。
   - 认知成本与抽象深度：拆分的目的是降低单个文件的复杂度与变更爆炸半径。若拆分导致追踪一条主逻辑线需要穿透过多浅层抽象与微型文件，说明破坏了内聚性，应按逻辑边界适度合并。
...
1. 正交职责隔离与防范浅层透传：
   - 包含 3 种以上正交职责（如 *流程编排 / 外部与 IO 接口 / 领域算法计算*）的模块必须拆分为独立文件，避免不同抽象层级的代码混杂在同一文件。
   - **区分正交职责与领域步骤**：“正交职责”指不同抽象层级的职责隔离，而非同一业务领域内部的连续处理步骤。
   - **防范中间人/浅层透传（Middleman Anti-pattern）**：避免创建仅做参数传递、无独立控制逻辑或无多方复用价值的薄壳模块。若上层编排与下层实现为 1:1 强绑定且无其他调用者，应优先保持同域内聚，避免无意义的浅层抽象拉长调用链路。
